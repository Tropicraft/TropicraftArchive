...
public class WorldGenBentRainforestTree extends TCGenBase{
	...
	public WorldGenBentRainforestTree(World world, Random random, boolean rainforestOnly) {
	...
	public boolean generate(int i, int j, int k) {
		...
		if(!checkBlockCircleLine(new int[] { randOffsetX1, height1, randOffsetZ1 }, new int[] { randOffsetX2, height2, randOffsetZ2 }, 1.3, 0, Arrays.asList(0, Block.tallGrass.blockID, Block.snow.blockID, TropicraftBlocks.tropicsLeaves.blockID, Block.leaves.blockID))) 
		{
			return false;
		}
		
		if(checkAndPlaceBlockCircleLine(new int[] { i, j, k }, new int[] { randOffsetX1, height1, randOffsetZ1 }, 1.3, 0, TropicraftBlocks.treeWood.blockID, 1,  Arrays.asList(0, Block.tallGrass.blockID, Block.snow.blockID, TropicraftBlocks.tropicsLeaves.blockID, Block.leaves.blockID)) == null)
		{
			return false;
		}
		
		ArrayList<int[]> trunk = placeBlockCircleLine(new int[] { randOffsetX1, height1, randOffsetZ1 }, new int[] { randOffsetX2, height2, randOffsetZ2 }, 1.3, 0, TropicraftBlocks.treeWood.blockID, 1);
	...
	public boolean genBranch(int[] block)
	{
		...
		if(checkAndPlaceBlockCircleLine(new int[] { ax, branchHeight + block[1], az}, new int[] { block[0], block[1], block[2] }, 1, 0, TropicraftBlocks.treeWood.blockID, 1, Arrays.asList(0, Block.tallGrass.blockID, Block.snow.blockID)) == null)
		genSphere(ax, branchHeight + block[1], az, rand.nextInt(2) + 2, TropicraftBlocks.tropicsLeaves.blockID, 3);
		return true;
	}
