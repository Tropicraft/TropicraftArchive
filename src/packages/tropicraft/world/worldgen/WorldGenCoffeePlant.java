...
public class WorldGenCoffeePlant extends WorldGenerator {
	...
	public boolean generate(World world, Random random, int x, int y, int z) {
		...
		world.setBlock(nx+viableDirection.offsetX, ny-1, nz+viableDirection.offsetZ, Block.waterStill.blockID, 0, 3);
		world.setBlock(nx, ny-1, nz, Block.tilledField.blockID, 7, 3);
		
		for (int i = 0; i < 3; ++i) {
			if (world.isAirBlock(nx, ny+i, nz)) {
				world.setBlock(nx, ny+i, nz, TropicraftBlocks.coffeePlant.blockID, 6, 3);
			} else {
				break;
			}
		}
