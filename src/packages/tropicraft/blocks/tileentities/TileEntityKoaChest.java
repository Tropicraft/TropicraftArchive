    ...
    implements IInventory
{
    ...
    public void updateList() {
    	int j;
    	for (j = 0; j < entIDs.length; j++) {
    		Entity ent = c_CoroAIUtil.getEntByPersistantID(worldObj, entIDs[j]);
    		if (ent == null) {
    			entIDs[j] = -1;
        		entRefs[j] = null;
    		} else if (ent.isDead) {
    ...
    public int getHomeKoaCount() {
        ...
        List<c_EnhAI> ents = this.worldObj.getEntitiesWithinAABB(ICoroAI.class, AxisAlignedBB.getAABBPool().getAABB((double)this.xCoord, (double)this.yCoord, (double)this.zCoord, (double)(this.xCoord + 1), (double)(this.yCoord + 1), (double)(this.zCoord + 1)).expand(dist, dist/2, dist));
	    ...
	    if (this.delay > 0)
	    {
	    	...
	    	if (this.needListUpdate && this.delay < 20) {
	    		for (int i = 0; i < entIDs.length; i++) {
	    			...
	    			Entity ent = c_CoroAIUtil.getEntByPersistantID(worldObj, entIDs[i]);
    ...
    public void checkForAdjacentChests()
    {
        ...
        if (worldObj.getBlockId(xCoord - 1, yCoord, zCoord) == TropicraftBlocks.koaChest.blockID)
        {
        ...
        if (worldObj.getBlockId(xCoord, yCoord, zCoord - 1) == TropicraftBlocks.koaChest.blockID)
        {
