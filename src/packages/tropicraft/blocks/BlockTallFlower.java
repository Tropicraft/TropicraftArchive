...
public class BlockTallFlower extends BlockTropicraft {
	...
	public void updateTick(World world, int i, int j, int k, Random random) {   
		...
		if (world.getBlockId(i, j, k) == this.blockID && (meta < 7) && world.isAirBlock(i, j + 1, k)) {
			int growth = meta;
			if (growth >= 6) {
				world.setBlock(i, j + 1, k, TropicraftBlocks.tallFlower.blockID, 9, 3);
				world.setBlockMetadataWithNotify(i, j, k, 7, 3);
	...
	@Override
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase doop, ItemStack stack) {
	...
	public boolean canBlockStay(World world, int i, int j, int k) {
		...
		if (world.getBlockId(i, j, k) == blockID && (world.getBlockMetadata(i, j, k) == 9 /*pineapple*/ || world.getBlockMetadata(i, j, k) == 15 /*iris*/ )) {
			belowCheck = (world.getBlockId(i, j - 1, k) == this.blockID && world.getBlockMetadata(i, j - 1, k) == 7);
		} else {
