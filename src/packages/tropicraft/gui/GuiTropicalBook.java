...
public class GuiTropicalBook extends GuiScreen {
	...
	public void addIcons() {
		...
		for (int entry = indexPage * book.entriesPerIndexPage(); entry < (indexPage + 1) * book.entriesPerIndexPage(); entry++) {
			...
			TropicraftUtils.bindTextureModGui(openTextureIndex);
			...
			if(is != null) {
				itemRenderer.renderItemIntoGUI(fontRenderer, mc.renderEngine, is, -(int) (129 * 1.5), -(int) (80 * 1.5) + (indexPosition) * 20);
			}
			GL11.glPopMatrix();
			indexPosition++;
		}
	...
	public void drawScreen(int i, int j, float f) {
		...
		if (indexPage == -1) {
			...
			GL11.glDisable(GL11.GL_LIGHTING);
			TropicraftUtils.bindTextureModGui(closedTextureIndex);
			...
			//if (book.isPageVisible(selectedIndex)) {
				...
				if (book.hasIndexIcons()) {
					...
					if(is != null) {
						itemRenderer.renderItemIntoGUI(fontRenderer, mc.renderEngine, is, 15, -73);
					}
					GL11.glPopMatrix();
				}
			//}
