...
public class ItemCoconut extends ItemTropicraft {
	...
	/**
	 * ID of block that should be spawned when coconut is right clicked
	 */
	...
	public ItemCoconut(int id, CreativeTabs tab, int mode) {
		super(id, tab);
		this.mode = mode;
		this.placedBlockID = ModIds.BLOCK_COCONUT_ID;
	}
	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer player)
	{
		if (mode == 1) { //coconut bomb
			...
			//	System.out.println("True! :D" + Tropicraft.coconutBombWhitelistedUsers.toString());
			}
		}
    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer player, World world, int i, int j, int k, int meta, float par8, float par9, float par10)
    {
        ...
        if (world.canPlaceEntityOnSide(this.placedBlockID, i, j, k, false, meta, (Entity)null, itemstack))
		{
			Block block = Block.blocksList[this.placedBlockID];
			int j1 = block.onBlockPlaced(world, i, j, k, meta, par8, par9, par10, 0);

			if (world.setBlock(i, j, k, this.placedBlockID, j1, 3))
			{
				if (world.getBlockId(i, j, k) == this.placedBlockID)
				{
					Block.blocksList[this.placedBlockID].onBlockPlacedBy(world, i, j, k, player, itemstack);
					Block.blocksList[this.placedBlockID].onPostBlockPlaced(world, i, j, k, j1);
				}
