...
import tropicraft.items.ItemTropicraftMulti;
...
public class ItemAshenMask extends ItemTropicraftArmor {
	...
	public String getArmorTexture(ItemStack stack, Entity entity, int slot,
			int layer) {
		return slot == 2 ? "/mods/TropicraftMod/textures/armor/mask_1.png"
				: slot == 0 || slot == 1 || slot == 3 ? "/mods/TropicraftMod/textures/armor/mask_1.png"
						: null;
	}
	...
	public void registerIcons(IconRegister iconRegistry) {
		...
		this.itemIcon = iconRegistry.registerIcon(ModInfo.ICONLOCATION
				+ getImageName());
		for (int i = 0; i < displayNames.length; i++) {
			images[i] = iconRegistry.registerIcon(ModInfo.ICONLOCATION
					+ imageNames[i]);
		}
	}
	...
	public void renderHelmetOverlay(ItemStack stack, EntityPlayer player,
		...
		GL11.glBindTexture(3553 /*
				...
				.getTexture("/mods/TropicraftMod/gui/maskblur.png"));
	...
	public String getImageName() {
		if (imageNames.length <= 0) {
			throw new IllegalArgumentException(
					"Length of names array must be > 0 in ItemTropicraftMulti");
		}
