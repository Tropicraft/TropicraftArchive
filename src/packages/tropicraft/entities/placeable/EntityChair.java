...
public class EntityChair extends Entity implements WindHandler
{
    ...
    /** Constant used in datawatcher to map to the color of the chair */
    ...
    public EntityChair(World world, double d, double d1, double d2, float f1, int damagedColor, EntityPlayer player)
	{
		...
		//chairYaw = rotationYaw;
	//	TropicraftMod.debugOut(getClass(), "Chair Rotation Yaw = " + ((Float)rotationYaw).toString());
    ...
    protected void entityInit()
    {
        ...
        this.dataWatcher.addObject(COLOR, new Integer(0));
	...
	public void setCurrentDamage(int dam) {
		dataWatcher.updateObject(17, Integer.valueOf(dam));
	}
    ...
    private float getAngleToPlayer(EntityPlayer player){
    	...
    	double subAngle = MathHelper.wrapAngleTo180_double(player.rotationYaw);
    	double angle = subAngle + (180 - subAngle)*2; //MathHelper.wrapAngleTo180_double(player.rotationYaw);
 //   	TropicraftMod.debugOut(getClass(), ((Double)MathHelper.wrapAngleTo180_double(player.rotationYaw)).toString());
    ...
    public boolean attackEntityFrom(DamageSource par1DamageSource, float par2)
    {
        ...
        else if (!this.worldObj.isRemote && !this.isDead)
        {
            ...
            if (this.getDamageTaken() > 40)
            {
                ...
                entityDropItem(new ItemStack(TropicraftItems.chair, 1, getColor()), 0.0F);
                this.setDead();
            }
    ...
    public void onUpdate()
    {
        ...
        if (var24 > 0.26249999999999996D && TropicraftConfig.comeSailAway)
        {
        ...
        if (this.worldObj.isRemote && this.field_70279_a)
        {
            ...
            {
                var6 = this.posX + (this.chairX - this.posX) / (double)this.chairPosRotationIncrements;
                var8 = this.posY + (this.chairY - this.posY) / (double)this.chairPosRotationIncrements;
                var26 = this.posZ + (this.chairZ - this.posZ) / (double)this.chairPosRotationIncrements;
                var12 = MathHelper.wrapAngleTo180_double(this.chairYaw - (double)this.rotationYaw);
                this.rotationYaw = (float)((double)this.rotationYaw + var12 / (double)this.chairPosRotationIncrements);
        ...
        else
        {
            ...
            if (this.riddenByEntity != null)
            {
            	if(TropicraftConfig.comeSailAway){
...
//                    }
            ...
            if (!this.worldObj.isRemote)
            {
                ...
                if (var16 != null && !var16.isEmpty())
                {
                    for (var27 = 0; var27 < var16.size(); ++var27)
                    {
                        ...
                        if (var18 != this.riddenByEntity && var18.canBePushed() && var18 instanceof EntityChair)
                        {
    ...
    public void updateRiderPosition()
    {
        if (this.riddenByEntity != null)
        {
//        	double d = Math.cos(((double)rotationYaw * 3.1415926535897931D) / 45D) * 0.5D;
//			double d1 = Math.sin(((double)rotationYaw * 3.1415926535897931D) / 45D) * 0.5D;
