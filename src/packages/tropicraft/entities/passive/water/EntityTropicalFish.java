...
public class EntityTropicalFish extends EntityTropicraftWaterMob {
	...
	public boolean interact(EntityPlayer entityplayer)
	{
		...
		if(entityplayer.getCurrentEquippedItem() == null || entityplayer.getCurrentEquippedItem().itemID != TropicraftItems.fishingNet.itemID){
			return false;
		}
		if(!entityplayer.inventory.hasItem(TropicraftItems.bucketTropicsWater.itemID)){
			return false;
		}
		else{
			...
			for(int i = 0 ; i < entityplayer.inventory.mainInventory.length; i ++ ){
				if(entityplayer.inventory.getStackInSlot(i) != null){
					if(entityplayer.inventory.getStackInSlot(i).itemID == TropicraftItems.bucketTropicsWater.itemID){
						entityplayer.inventory.mainInventory[i] = new ItemStack(TropicraftItems.fishBucket, 1, getColor());
						this.setDead();
