  ...
  private static final short MAX_HIT_TIME = 10 * 20;	//number of seconds desired * number of ticks per second = number of ticks total 8)
	...
	public void onUpdate() {
		...
		if (getDamage() == 0) {
			...
			if (!worldObj.isRemote && ridingEntity != null && ridingEntity instanceof EntityLiving && this.getFlag(2) && !(((EntityLiving)ridingEntity).getActivePotionEffects().size() > 0) && !getIsHit() && getHitTimer() > 0) {
				setIsHit(true);
