...
public class EntityTameBall extends EntityThrowableUsefull
{
	...
	public EntityTameBall(World world, EntityLiving entityliving)
	{
		...
		if (movingobjectposition.entityHit != null)
		{
			if (!worldObj.isRemote)
			{
				...
				if (movingobjectposition.entityHit instanceof ICoroAI && getThrower() instanceof EntityPlayer) {
					...
					if (!tamable.isTame()) {
						tamable.tameBy(((EntityPlayer)getThrower()).username);
						setDead();
					}
				}
			}
	...
	public void tickAnimate() {
		...
		for (int i = 0; i < amount; i++)
        {
			...
			double speed = 0.01D;
			EntityRotFX entityfx = pm.spawnNewParticle(worldObj, ParticleRegistry.smoke, posX + rand.nextDouble(), posY + 0.2D + rand.nextDouble() * 0.2D, posZ + rand.nextDouble(), (rand.nextDouble() - rand.nextDouble()) * speed, 0.03D, (rand.nextDouble() - rand.nextDouble()) * speed);
        	pm.setParticleRandoms(entityfx, true, true);
        	pm.setParticleFire(entityfx);
			entityfx.setMaxAge(120+rand.nextInt(90));
			entityfx.particleScale = 0.55F + 0.2F * rand.nextFloat();
			entityfx.spawnAsWeatherEffect();
