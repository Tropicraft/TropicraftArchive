...
public class EntityTropicraftLeafballNew extends EntityThrowableUsefull
{
	...
	public EntityTropicraftLeafballNew(World world, EntityLiving entityliving)
	{
	...
	protected void onImpact(MovingObjectPosition movingobjectposition)
	{
		...
		if (movingobjectposition.entityHit != null)
		{
			...
			{
				byte byte0 = 2;
				if (movingobjectposition.entityHit instanceof c_EnhAI && (getThrower() == null || getThrower() instanceof c_EnhAI)) {
					if (getThrower() != null && ((c_EnhAI) getThrower()).dipl_team != ((c_EnhAI) movingobjectposition.entityHit).dipl_team) {
						movingobjectposition.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, getThrower()), byte0);
					} else {

					}
				} else if (movingobjectposition.entityHit instanceof ICoroAI && (getThrower() == null || getThrower() instanceof ICoroAI)) {
	...
	public void tickDeath() {
		if (!hasDeathTicked) {
			...
			for (int i = 0; i < amount; i++)
	        {
	        	...
	        	EntityRotFX entityfx = new EntityIconFX(worldObj, posX, posY, posZ, TropicraftItems.leafBall.getIconFromDamage(0), Minecraft.getMinecraft().renderEngine);
