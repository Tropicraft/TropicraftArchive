...
public class JobAttackTargetShare extends JobBase {
	...
	public boolean hookHit(DamageSource ds, int damage) {
		...
		if (isEnemy(target) && shareTriggerCooldown == 0) {
			...
			List<EntityLiving> notifyList = ent.worldObj.getEntitiesWithinAABB(classToShareWith, ent.boundingBox.expand(rangeTargetShare, rangeTargetShare, rangeTargetShare));
			
			for (int i = 0; i < notifyList.size(); i++) {
				EntityLiving entN = notifyList.get(i);
