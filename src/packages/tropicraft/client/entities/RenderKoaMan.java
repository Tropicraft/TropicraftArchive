...
public class RenderKoaMan extends RenderLiving
{
			...
			float f, float f1)
	{
		...
		if(entityliving instanceof EntityKoaBase)
		{
			EntityKoaBase koa = (EntityKoaBase)entityliving;
			if(koa.inWaterTick > 0 && koa.waterOffsetY > -1.2f){
				koa.waterOffsetY -= 0.1f;
			}
			if(koa.outWaterTick > 10 && koa.waterOffsetY < 0F){
				koa.waterOffsetY += 0.05f;
				if(koa.waterOffsetY > 0F){
					koa.waterOffsetY = 0f;
				}
			}
			GL11.glTranslatef(0f, koa.waterOffsetY, 0f);
			
			//mainModel.bipedLeftLeg.rotateAngleX++;
			float offset = (float) Math.sin(entityliving.ticksExisted * 0.025f)/2;
			if(offset > 0){
				offset = -offset;
			}
			if(koa.inWaterTick > 2 || koa.outWaterTick < 10){
			mainModel.bipedLeftLeg.rotateAngleZ=offset;
			mainModel.bipedRightLeg.rotateAngleZ=-offset;
			//mainModel.bipedBody.rotateAngleY= offset;
			}else{
				mainModel.bipedLeftLeg.rotateAngleZ=0;
				mainModel.bipedRightLeg.rotateAngleZ=0;
			}
		}
	...
	protected void renderName(EntityLiving entitykoa, double d, double d1, double d2)
	{
		...
		if(Minecraft.isGuiEnabled() && entitykoa != renderManager.livingPlayer)
		{
			...
			String s = entitykoa.func_94057_bL();
			...
			float f, float f1)
	{
		...
		renderengine.bindTexture("%clamp%/misc/hmm.png");
