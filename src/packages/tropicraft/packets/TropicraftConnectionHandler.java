...
public class TropicraftConnectionHandler implements IConnectionHandler {
	...
	public void playerLoggedIn(Player player, NetHandler netHandler,
		...
		if (scm.playerEntityList.size() == 1) {
			Tropicraft.proxy.getUpsideDownUsernames().clear();
		}
		
		NetServerHandler nsh = (NetServerHandler) netHandler;
		nsh.sendPacketToPlayer(TropicraftPacketHandler.getSnareTrapPacket(true, true, Tropicraft.proxy.getUpsideDownUsernames()));
	}
