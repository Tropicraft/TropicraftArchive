...
public class JobTrade extends JobBase {
	...
	public boolean interact(EntityPlayer ent) {
		...
		if (is != null) {
			...
			if (ValuedItems.getItemCost(is.getItem()) != 0) {
	...
	public void convertOfferingsToCurrency(int newCredit) {
		...
		while (leftToConvert > TropicraftMod.shellCommon1.getItemStackLimit()) {
			offeredItems.add(new ItemStack(TropicraftMod.shellCommon1, TropicraftMod.shellCommon1.getItemStackLimit()));
			//activeTrader.inventory.addItemStackToInventory();
			leftToConvert -= TropicraftMod.shellCommon1.getItemStackLimit();
		}
		
		if (leftToConvert > 0) {
			offeredItems.add(new ItemStack(TropicraftMod.shellCommon1, leftToConvert));
		}
	}
	...
	public void tick() {
		...
		//TEMP!!!!!!!!
		idTradeBlock = TropicraftMod.tradeBlock.blockID;
			...
			for(int j = 0; j < list.size(); j++)
	        {
	            ...
	            if(entity1 instanceof EntityItemFrame) {
	            	if (((EntityItemFrame)entity1).getDisplayedItem() == null) {
	            		((EntityItemFrame)entity1).setDisplayedItem(new ItemStack(ValuedItems.getItemBuyable(ent.worldObj.rand.nextInt(ValuedItems.buyables.size()))));
	            	}
	            }
	        }
		}
	...
	public void tradeConfirmCallback() {
		...
		int cost = ValuedItems.getBuyableItemCostByIndex(tradePlate.itemIndex);
		...
		if (credit >= cost) {
			...
			ItemStack is = new ItemStack(ValuedItems.getItemBuyable(tradePlate.itemIndex), ValuedItems.getBuyItemCount(ValuedItems.getItemBuyable(tradePlate.itemIndex)));
