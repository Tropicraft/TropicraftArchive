...
public class RenderDart extends Render {
    ...
    public void renderArrow(EntityDart entityarrow, double d, double d1, double d2,
            float f, float f1) {
        if (entityarrow.ridingEntity != null) {
            return;
        }
        if (entityarrow.prevRotationYaw == 0.0F && entityarrow.prevRotationPitch == 0.0F) {
            return;
        }
        loadTexture("/tropicalmod/darts.png");
        GL11.glPushMatrix();
        ...
        float f11 = (float) entityarrow.dartShake - f1;
        if (f11 > 0.0F) {
