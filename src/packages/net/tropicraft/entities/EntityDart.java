...
public class EntityDart extends Entity {
	...
	public EntityDart(World world, EntityLiving entityliving, float f, int damage) {
		super(world);
		prevTasks = null;
		xTile = -1;
		yTile = -1;
		...
		ticksInAir = 0;
		owner = entityliving;
		doesDartBelongToPlayer = entityliving instanceof EntityPlayer;
		setSize(0.5F, 0.5F);
		...
		Fov = TropicraftMod.proxy.getFOV();
		sinc = 0.0D;
		k = 0;
		hasRidden = false;
		setDamage(damage);
	...
	public void onUpdate() {
		...
		if (list != null) {
			for (int l = 0; l < list.size(); l++) {
				...
				if (!entity1.canBeCollidedWith() || entity1 == owner && ticksInAir < 10) {
					continue;
				}
				float f4 = 0.3F;
		...
		if (movingobjectposition != null) {
			if (movingobjectposition.entityHit != null) {
				...
				if (movingobjectposition.entityHit instanceof EntityEIH || movingobjectposition.entityHit instanceof EntityEnderman) { 
					motionX *= -.01D;
					motionY *= .5D;
					motionZ *= -.01D;
					rotationYaw += 180F;
				...
				if (movingobjectposition.entityHit.attackEntityFrom(DamageSource.causeMobDamage(owner), 0)) {
					...
					if (entity instanceof EntityLiving) {
						moveSpeedAct = EntityDartHelper.getEntityMoveSpeed((EntityLiving) entity);
					}

					mountEntity(entity);
					movingobjectposition.entityHit.velocityChanged = false;

					worldObj.playSoundAtEntity(this, "darthit", 1.0F, 1.2F / (rand.nextFloat() * 0.2F + 0.9F));
		...
		if (getDamage() == 0) {
				...
				if (k < 135) {
					...
					if (ridingEntity instanceof EntityPlayer) {
						if(worldObj.isRemote && ((EntityPlayer)ridingEntity).username.equals(getClientUsername())) {
							TropicraftMod.proxy.setFOV(Fov);
						}
					}
					setDead();
				}
			...
			if (!worldObj.isRemote && ridingEntity != null && ridingEntity instanceof EntityLiving && this.getFlag(2) && !((EntityLiving)ridingEntity).getHasActivePotion() && !getIsHit() && getHitTimer() > 0) {
				setIsHit(true);
				...
				((EntityLiving) ridingEntity).addPotionEffect(new PotionEffect(potions[getDamage()], MAX_HIT_TIME, 1));
	...
	public void onCollideWithPlayer(EntityPlayer entityplayer) {
		...
		if (inGround && doesDartBelongToPlayer && dartShake <= 0 && entityplayer.inventory.addItemStackToInventory(new ItemStack(TropicraftMod.paraDart, 1))) {
	...
	public void trollFOV(EntityPlayer entity) {
		...
		if (k > 125) {
			if (herp == -20F && entity.worldObj.isRemote) {
				herp = (TropicraftMod.proxy.getFOV() - Fov) / 10;
			}
			if (entity.worldObj.isRemote) {
				trollov = TropicraftMod.proxy.getFOV() - herp;
			}
		}

		if (entity.worldObj.isRemote)
			TropicraftMod.proxy.setFOV(trollov);
