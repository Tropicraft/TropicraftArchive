...
public class EntityHangingTropicraft extends EntityItemFrame {

	/**
	 * Should drop the item held in this frame, used only for koa for now.
	 */
	...
	public EntityHangingTropicraft(World par1World) {
		super(par1World);
	//	this.shouldDropContents = false;
		setShouldDropContents(false);
	}

	public EntityHangingTropicraft(World par1World, int par2, int par3,
			int par4, int par5, boolean shouldDropContents) {
	...
	public void dropItemStack() {
		...
		if (!getShouldDropContents()) {
			this.entityDropItem(new ItemStack(TropicraftMod.koaFrame), 0.0F);	//drop only item frame
		} else {
			this.entityDropItem(new ItemStack(TropicraftMod.tropiFrame), 0.0F);	//drop only item frame
        ...
        else
        {
            ...
            for (int var6 = 0; var6 < var1; ++var6)
            {
                for (int var7 = 0; var7 < var2; ++var7)
                {
                    ...
                    if (!var8.isSolid() && blawk != null && !(blawk.blockID == TropicraftMod.bambooBlock.blockID))
                    {
