...
public class EntityTropicalFish extends EntityTropicraftWaterMob {
	...
	public EntityTropicalFish(World world) {
		super(world);
		this.texture = "/tropicalmod/tropicalFish.png";
		targetHook = false;
		isLeader = true;
		inSchool = false;
		leader = null;		
	...
	public void checkForLeader(){
		...
		for(Object ent : list){
			...
			if(((EntityTropicalFish)ent).getColor() == this.getColor()){
				if(entityId > ((Entity)ent).entityId){
	...
	public boolean interact(EntityPlayer entityplayer)
	{
		...
		if(entityplayer.getCurrentEquippedItem() == null || entityplayer.getCurrentEquippedItem().itemID != TropicraftMod.fishingNet.shiftedIndex){
			return false;
		}
		if(!entityplayer.inventory.hasItem(TropicraftMod.bucketTropicsWater.shiftedIndex)){
			return false;
		}
		else{
			...
			for(int i = 0 ; i < entityplayer.inventory.mainInventory.length; i ++ ){
				if(entityplayer.inventory.getStackInSlot(i) != null){
					if(entityplayer.inventory.getStackInSlot(i).itemID == TropicraftMod.bucketTropicsWater.shiftedIndex){
						entityplayer.inventory.mainInventory[i] = new ItemStack(TropicraftMod.fishBucket, 1, getColor());
						this.setDead();
