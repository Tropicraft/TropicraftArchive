...
public class EntityPoisonousTreeFrogRed extends EntityAnimal implements IMob {

	public EntityPoisonousTreeFrogRed(World world) {
		super(world);
		texture = "/tropicalmod/redtreefrogtexture.png";
		setSize(0.9F, 1.3F);
		health = 4;
		savedhealth = health;
		onWall = false;
	...
	public void onLivingUpdate() {
		...
		savedhealth = health;
		/*
		 * if(onWall && System.currentTimeMillis() >= timer) { onWall = false;
		 * motionX = savedX; motionY = -0.35699998688697815D; motionZ = savedZ;
		 * } if(onWall) { timer = System.currentTimeMillis() + 50000L; savedX =
		 * motionX; savedY = motionY; savedZ = motionZ; motionX = 0.0D; motionY
		 * = 0.0D; motionZ = 0.0D; } if(isCollidedHorizontally && !onWall &&
		 * worldObj.getBlockId((int)posX, (int)(posY - 1.5D), (int)posZ) == 0 &&
		 * rand.nextInt(4) > 2) { if(worldObj.getBlockId((int)posX - 1,
		 * (int)posY, (int)posZ) == 0) { side = 1; } else
		 * if(worldObj.getBlockId((int)posX, (int)posY, (int)posZ - 1) == 0) {
		 * side = 2; } else if(worldObj.getBlockId((int)posX + 1, (int)posY,
		 * (int)posZ) == 0) { side = 3; } else if(worldObj.getBlockId((int)posX,
		 * (int)posY, (int)posZ + 1) == 0) { side = 4; } onWall = true; timer =
		...
		if (newPosRotationIncrements > 0) {
			...
			List list1 = worldObj.getCollidingBoundingBoxes(this,
					boundingBox.getOffsetBoundingBox(0.03125D, 0.0D, 0.03125D));
			if (list1.size() > 0) {
				double d4 = 0.0D;
	...
	protected void attackEntity(Entity entity, float f) {
		if (f < 4F && worldObj.difficultySetting != 0) {
			...
			if (attackTime == 0 && !worldObj.isRemote
					&& worldObj.difficultySetting != 0) {
				EntityPoisonBlot entitypoisonblot = new EntityPoisonBlot(
						worldObj, this);
				entitypoisonblot.posY += 1.3999999761581421D;
				double d2 = (entity.posY + (double) entity.getEyeHeight())
						- 0.20000000298023224D - entitypoisonblot.posY;
				float f1 = MathHelper.sqrt_double(d * d + d1 * d1) * 0.2F;
				worldObj.playSoundAtEntity(this, "frogspit", 1.0F,
						1.0F / (rand.nextFloat() * 0.4F + 0.8F));
				worldObj.spawnEntityInWorld(entitypoisonblot);
				entitypoisonblot.setArrowHeading(d, d2 + (double) f1, d1, 0.6F,
						12F);
				attackTime = 50;
			}
