    ...
    implements IInventory
{
	...
	public int housePop = 6;
	public int housePop_hunters = 4;
	public int housePop_fishers = 2;
	
	public int[] entIDs;
	public EntityKoaMemberNew[] entRefs;
	private int hunters;
	private int fishers;
    ...
    public TileEntityKoaChest()
    {
        ...
        entRefs = new EntityKoaMemberNew[housePop];
    ...
    public void spawnKoa(World world) {
        ...
        while ((hunters < housePop_hunters || fishers < housePop_fishers) && spawnTry < 20)
        {
        	spawnTry++;
        	EntityKoaManly var2 = new EntityKoaManly(this.worldObj);
	        ...
	        if (var2 != null)
	        {
	            ...
	            if (getCanSpawnHere(var2))
	            {
	            	...
	            	var2.homeX = this.xCoord;
	            	var2.homeY = this.yCoord;
	            	var2.homeZ = this.zCoord;
    ...
    public void addToList(EntityKoaMemberNew ent) {
    	int j;
    ...
    public void updateList() {
    	int j;
    	for (j = 0; j < entIDs.length; j++) {
    		c_EnhAI ent = TropicraftMod.proxy.getEntByPersistantID(worldObj, entIDs[j]);
    		if (ent == null) {
    			entIDs[j] = -1;
        		entRefs[j] = null;
    		} else if (ent.isDead) {
    ...
    public int getHomeKoaCount() {
        ...
        List<c_EnhAI> ents = this.worldObj.getEntitiesWithinAABB(c_EnhAI.class, AxisAlignedBB.getAABBPool().addOrModifyAABBInPool((double)this.xCoord, (double)this.yCoord, (double)this.zCoord, (double)(this.xCoord + 1), (double)(this.yCoord + 1), (double)(this.zCoord + 1)).expand(dist, dist/2, dist));
        ...
        for (int i = 0; i < ents.size(); i++) {
        	...
        	for (j = 0; j < entIDs.length; j++) {
	        	...
	        	Entity ent = ents.get(i);//TropicraftMod.proxy.getEntByID(entIDs[j]);
	        	if (ent instanceof EntityKoaMemberNew && ent != null && entIDs[j] == ((EntityKoaMemberNew)ent).entID) {
	        		existing++;
	        		//System.out.println("derp: " + existing);
	        		if (((EntityKoaMemberNew)ent).job.priJob == EnumJob.HUNTER) {
	        			hunters++;
	        		}
	            	if (((EntityKoaMemberNew)ent).job.priJob == EnumJob.FISHERMAN) {
	            		fishers++;
	            	} else {
	            		int dsdf = 0;
	            	}
    ...
    {
        return this.worldObj.checkIfAABBIsClear(ent.boundingBox) && ent.worldObj.getCollidingBoundingBoxes(ent, ent.boundingBox).isEmpty() && !ent.worldObj.isAnyLiquid(ent.boundingBox);
    }
	    ...
	    if (this.delay > 0)
	    {
	    	...
	    	if (this.needListUpdate && this.delay < 20) {
	    		for (int i = 0; i < entIDs.length; i++) {
	    			...
	    			Entity ent = TropicraftMod.proxy.getEntByPersistantID(worldObj, entIDs[i]);
    ...
    public void checkForAdjacentChests()
    {
        ...
        if (worldObj.getBlockId(xCoord - 1, yCoord, zCoord) == TropicraftMod.koaChest.blockID)
        {
            adjacentChestXNeg = (TileEntityKoaChest)worldObj.getBlockTileEntity(xCoord - 1, yCoord, zCoord);
        }
        if (worldObj.getBlockId(xCoord + 1, yCoord, zCoord) == TropicraftMod.koaChest.blockID)
        {
            adjacentChestXPos = (TileEntityKoaChest)worldObj.getBlockTileEntity(xCoord + 1, yCoord, zCoord);
        }
        if (worldObj.getBlockId(xCoord, yCoord, zCoord - 1) == TropicraftMod.koaChest.blockID)
        {
            adjacentChestZNeg = (TileEntityKoaChest)worldObj.getBlockTileEntity(xCoord, yCoord, zCoord - 1);
        }
        if (worldObj.getBlockId(xCoord, yCoord, zCoord + 1) == TropicraftMod.koaChest.blockID)
        {
            adjacentChestZPosition = (TileEntityKoaChest)worldObj.getBlockTileEntity(xCoord, yCoord, zCoord + 1);
        }
    ...
    public void openChest()
    {
        ...
        this.worldObj.addBlockEvent(this.xCoord, this.yCoord, this.zCoord, TropicraftMod.koaChest.blockID, 1, this.numUsingPlayers);
    }
