...
public class BlockFluidLAPI extends Block {

	public int liquidID;

	public BlockFluidLAPI(int par1, int liqID, Material par2Material) {
		super(par1, liqID, par2Material);
		this.liquidID = liqID;
		float var3 = 0.0F;
		float var4 = 0.0F;
		this.setBlockBounds(0.0F + var4, 0.0F + var3, 0.0F + var4, 1.0F + var4,
				1.0F + var3, 1.0F + var4);
		this.setTickRandomly(true);
		this.setTextureFile("/lapi/lapiterrain.png");
		disableStats();
	...
	public int colorMultiplier(IBlockAccess iblockaccess, int i, int j, int k) {
		int d = LAPI.getColorValue(liquidID);
	...
	protected int getFlowDecay(World par1World, int par2, int par3, int par4) {
		return par1World.getBlockMaterial(par2, par3, par4) == this.blockMaterial ? par1World
	...
	public boolean isBlockSolid(IBlockAccess par1IBlockAccess, int par2,
		...
		return var6 == this.blockMaterial ? false : (par5 == 1 ? true
	...
	private Vec3 getFlowVector(IBlockAccess par1IBlockAccess, int par2,
		...
		int var6 = this.getEffectiveFlowDecay(par1IBlockAccess, par2, par3,
				par4);
		...
		for (int var7 = 0; var7 < 4; ++var7) {
			...
			int var11 = this.getEffectiveFlowDecay(par1IBlockAccess, var8,
					par3, var10);
			int var12;
			...
			if (var11 < 0) {
				if (!par1IBlockAccess.getBlockMaterial(var8, par3, var10)
						.blocksMovement()) {
					var11 = this.getEffectiveFlowDecay(par1IBlockAccess, var8,
							par3 - 1, var10);

					if (var11 >= 0) {
						var12 = var11 - (var6 - 8);
						var5 = var5.addVector((double) ((var8 - par2) * var12),
		...
		if (par1IBlockAccess.getBlockMetadata(par2, par3, par4) >= 8) {
			...
			if (var13
					|| this.isBlockSolid(par1IBlockAccess, par2, par3,
							par4 - 1, 2)) {
				var13 = true;
			}
			...
			if (var13
					|| this.isBlockSolid(par1IBlockAccess, par2 - 1, par3,
							par4, 4)) {
				var13 = true;
			}
			...
			if (var13
					|| this.isBlockSolid(par1IBlockAccess, par2, par3 + 1,
							par4 - 1, 2)) {
				var13 = true;
			}
			...
			if (var13
					|| this.isBlockSolid(par1IBlockAccess, par2 - 1, par3 + 1,
							par4, 4)) {
				var13 = true;
			}
	...
	public int tickRate() {
		return this.blockMaterial == Material.water ? 5
				: (this.blockMaterial == Material.lava ? 30 : 0);
	}
	...
	public int getMixedBrightnessForBlock(IBlockAccess par1IBlockAccess,
			int par2, int par3, int par4) {
		int var5 = par1IBlockAccess.getLightBrightnessForSkyBlocks(par2, par3,
				par4, 0);
		int var6 = par1IBlockAccess.getLightBrightnessForSkyBlocks(par2,
				par3 + 1, par4, 0);
		int var7 = var5 & 255;
		int var8 = var6 & 255;
	...
	public void randomDisplayTick(World par1World, int par2, int par3,
			...
			for (var6 = 0; var6 < 0; ++var6) {
				...
				if (par1World.getBlockMaterial(var8, par3, var9) == Material.air
						&& (par1World.getBlockMaterial(var8, par3 - 1, var9)
								...
								.isLiquid())) {
					float var10 = 0.0625F;
					double var11 = (double) ((float) par2 + par5Random
							.nextFloat());
					double var13 = (double) ((float) par3 + par5Random
							.nextFloat());
					double var15 = (double) ((float) par4 + par5Random
							.nextFloat());
		...
		if (this.blockMaterial == Material.water && par5Random.nextInt(64) == 0) {
			...
			if (var6 > 0 && var6 < 8) {
				par1World.playSound((double) ((float) par2 + 0.5F),
						...
						(double) ((float) par4 + 0.5F), "liquid.water",
		...
		if (this.blockMaterial == Material.lava
			...
			if (par5Random.nextInt(200) == 0) {
				par1World.playSound((double) par2, (double) par3,
						(double) par4, "liquid.lava",
						0.2F + par5Random.nextFloat() * 0.2F,
						0.9F + par5Random.nextFloat() * 0.15F, false);
			}
		}
	...
	public static double getFlowDirection(IBlockAccess par0IBlockAccess,
			int par1, int par2, int par3, Material par4Material) {
		Vec3 var5 = null;

		if (par4Material == Material.water) {
			var5 = ((BlockFluidLAPI) Block.waterMoving).getFlowVector(
					par0IBlockAccess, par1, par2, par3);
		}

		if (par4Material == Material.lava) {
			var5 = ((BlockFluidLAPI) Block.lavaMoving).getFlowVector(
					par0IBlockAccess, par1, par2, par3);
		}
	...
	protected void triggerLavaMixEffects(World par1World, int par2, int par3,
			int par4) {
		par1World
				.playSoundEffect((double) ((float) par2 + 0.5F),
						(double) ((float) par3 + 0.5F),
