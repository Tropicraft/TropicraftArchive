...
public class TextureTropicsPortalFX extends TextureFX
{

    public TextureTropicsPortalFX()
    {
        super(TropicraftMod.tropicraftPortal.blockIndexInTexture);
        portalTickCounter = 0;
        ...
        for(int i = 0; i < 32; i++)
        {
            for(int j = 0; j < 16; j++)
            {
                for(int k = 0; k < 16; k++)
                {
                    ...
                    int i2 = k * 16 + j;
                    portalTextureData[i][i2 * 4 + 0] = (byte)0;//j1;
                    portalTextureData[i][i2 * 4 + 1] = (byte)k1;
                    portalTextureData[i][i2 * 4 + 2] = (byte) ((byte)i1+(byte)3);
                    portalTextureData[i][i2 * 4 + 3] = (byte)211;
                }
    ...
    public void onTick()
    {
        ...
        for(int i = 0; i < 256; i++)
        {
            ...
            imageData[i * 4 + 0] = (byte)0;
            imageData[i * 4 + 1] = (byte)(k);
            imageData[i * 4 + 2] = (byte)(l+20);
            imageData[i * 4 + 3] = (byte)150;
        }
