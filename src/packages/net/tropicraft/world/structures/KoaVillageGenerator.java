...
public class KoaVillageGenerator {
    ...
    private static final List<Integer> waterBlockIDs = Arrays.asList(Block.waterMoving.blockID, Block.waterStill.blockID, TropicraftMod.waterMovingTropics.blockID, TropicraftMod.waterStillTropics.blockID);
    private static final List<Integer> sandBlockIDs = Arrays.asList(Block.sand.blockID, TropicraftMod.purifiedSand.blockID);
    ...
    private void generateSexy(World world, Random random, int i, int j, int k, int dir) {
        ...
        int y = j - 1;
        int z = k;
        Block thatchStair = TropicraftMod.thatchStairs;
        ...
        for (int a = 0; a < 6; a++) {
            ...
            if (sandBlockIDs.contains(world.getBlockId(x + xOffset, yLoc, z + zOffset)) &&
                ...
                placeWithMetadata(world, x + xOffset, yLoc + 1, z + zOffset, TropicraftMod.tikiTorch, 1, false);
                placeWithMetadata(world, x + xOffset, yLoc + 2, z + zOffset, TropicraftMod.tikiTorch, 1, false);
                placeWithMetadata(world, x + xOffset, yLoc + 3, z + zOffset, TropicraftMod.tikiTorch, 0, false);
            }
        }
    ...
    protected void placeWithMetadata(World world, int i, int j, int k, Block block, int meta, boolean notify) {
        if (notify) {
            world.setBlockAndMetadataWithNotify(i, j, k, block.blockID, meta);
        } else {
            world.setBlockAndMetadata(i, j, k, block.blockID, meta);
        }
    }
    ...
    private void generateHuts(World world, Random random) {
        ...
        for (KoaBridgePiece kbp : bridgePieceList) {
            if (kbp.canBuildHere) {
                ...
                if (random.nextFloat() < 0.4F) {
                    ...
                    //KoaVillageHut kvm = (new KoaVillageHut(village, world, kbp.x, kbp.y, kbp.z, coordsOfAdjacentAir(world, kbp.x, kbp.y, kbp.z)[0], coordsOfAdjacentAir(world, kbp.x, kbp.y, kbp.z)[1], coordsOfAdjacentAir(world, kbp.x, kbp.y, kbp.z)[2],
                    //        coordsOfAdjacentAir(world, kbp.x, kbp.y, kbp.z)[3]));
        ...
        for (ChunkCoordinates ksp : this.village.spawnPoints) {
            place(world, ksp.posX, ksp.posY, ksp.posZ, TropicraftMod.bambooChest);
        }
    }
    ...
    private void generateBridge(World world, Random random, int i, int j, int k, int dir, int length) {
        Block bridgebl = TropicraftMod.thatchBlock;
    ...
    private int getMaxDistance(World world, int i, int j, int k, int direction) {
        int max = 0;
        int id = TropicraftMod.thatchBlock.blockID;
