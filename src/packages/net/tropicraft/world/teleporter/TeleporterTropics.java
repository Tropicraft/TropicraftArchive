...
{   
    private int portalBlockID = TropicraftMod.tropicraftPortal.blockID;
    private int portalWallID = TropicraftMod.portalWall.blockID;
    private static final List<Integer> validBlockIDs = Arrays.asList(Block.sand.blockID, TropicraftMod.purifiedSand.blockID, Block.grass.blockID, Block.dirt.blockID);
    
    /**
     * World instance for Teleporter classes
     */
    ...
    public void placeInPortal(Entity entity, double d, double d2, double d3, float f)
    {
        ...
        {
        	func_85188_a(entity);	//createPortal
    ...
    public boolean placeInExistingPortal(Entity entity, double d, double d2, double d3, float f)
    {
        ...
        if (closestPortal >= 0.0D)
        {
            ...
            if (entity instanceof EntityPlayer) {
                ...
                if (world.provider instanceof WorldProviderTropics) {
                    if (!player.inventory.hasItem(TropicraftMod.encTropica.shiftedIndex)) {
                        ...
                        for (int searchX = -3; searchX < 4; searchX++) {
                            for (int searchZ = -3; searchZ < 4; searchZ++) {
                                for (int searchY = -3; searchY < 4; searchY++) {
                                    if (world.getBlockId(chestX + searchX, worldSpawnY + searchY, chestZ + searchZ) == TropicraftMod.bambooChest.blockID) {
                        ...
                        if (chest!= null && chest.isUnbreakable()) {
                            ...
                            for (int inv = 0; inv < chest.getSizeInventory(); inv++) {
                                ...
                                if (stack != null && stack.getItem() == TropicraftMod.encTropica) {
                            ...
                            if (!hasEncyclopedia) {
                                for (int inv = 0; inv < chest.getSizeInventory(); inv++) {
                                    ItemStack stack = chest.getStackInSlot(inv);
                                    if (stack == null) {
                                        chest.setInventorySlotContents(inv, new ItemStack(TropicraftMod.encTropica, 1));
                                        break;
                                    }
                                }
                            }
...
nextCoords:
    ...
    public void buildTeleporterAt(int x, int y, int z, Entity entity) {        
            ...
            for (int zOffset = -2; zOffset <= 2; zOffset++) {
                for (int xOffset = -2; xOffset <= 2; xOffset++) {
                    ...
                    if (yOffset == -7) {
                        ...
                        world.setBlockWithNotify(blockX, blockY, blockZ, portalWallID);
                    } else if (yOffset > 0) {
                        ...
                        if (isWall) {
                            ...
                            world.setBlockAndMetadataWithNotify(blockX, blockY, blockZ, portalBlockID, metadata);
                        }
                    }
                    ...
                    //	System.out.println("I found waldo!" + blockX + " " + blockY + " " + blockZ + " Dimension=" + world.provider.dimensionId);
                        world.setBlockAndMetadataWithNotify(blockX, blockY + 1, blockZ, TropicraftMod.tikiTorch.blockID, 1);
                        world.setBlockAndMetadataWithNotify(blockX, blockY + 2, blockZ, TropicraftMod.tikiTorch.blockID, 1);
                        world.setBlockAndMetadataWithNotify(blockX, blockY + 3, blockZ, TropicraftMod.tikiTorch.blockID, 0);
                    }
        ...
        if (world.provider instanceof WorldProviderTropics) {
            world.setBlockAndMetadataWithNotify(x + 2, y + 1, z, TropicraftMod.bambooChest.blockID, 1);
