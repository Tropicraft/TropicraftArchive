...
public class WorldGenMedRTree extends TCGenBase {
	...
	private int woodBlockId = TropicraftMod.tropicalWood.blockID;
	private int leafBlockId = TropicraftMod.tropicLeaves.blockID;
	...
	{	
		...
		if(gen)
		{
			if(BiomeGenTropicraft.rainforestBiomes.contains(biome))
			{
				return false;
			}
		}
		...
		for(int y = j; y <= j + height; y++)
		{
			...
			worldObj.setBlockAndMetadata(tx, y, tz, woodBlockId, 1);
			worldObj.setBlockAndMetadata(tx - 1, y, tz, woodBlockId, 1);
			worldObj.setBlockAndMetadata(tx + 1, y, tz, woodBlockId, 1);
			worldObj.setBlockAndMetadata(tx, y, tz - 1, woodBlockId, 1);
			worldObj.setBlockAndMetadata(tx, y, tz + 1, woodBlockId, 1);
			if(y - j == bs1)
			{
