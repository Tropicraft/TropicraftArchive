...
public class EntTornado extends Entity implements IEntityAdditionalSpawnData
{
    ...
    public boolean tryRip(int tryX, int tryY, int tryZ, boolean notify)
    {
        ...
        if (!WeatherMod.Storm_Tornado_grabBlocks) return true;
        ...
        if (worldObj.isRemote) {
	        if (WeatherMod.particleCount < WeatherMod.Storm_Tornado_maxParticles)
	        {
	            ...
	            for (int i = 0; i < itCount; i++)
	            {
	                ...
	                {
	                    WeatherMod.proxy.newAnimParticle("AnimTexFX", worldObj, this, tryX2, spawnYOffset, tryZ2, 0, 0, 0, 9.5D, WeatherMod.effWindAnimID, colorID, scale);
	                    //derp
