...
import weather.renderer.EntityFireflyFX;
import weather.renderer.EntityRotFX;
import weather.renderer.EntitySnowFX;
import weather.renderer.EntityTexBiomeColorFX;
...
@Mod(modid = "WeatherMod", name = "Weather and Tornadoes", version = "v1.3")
...
public class WeatherMod implements Runnable
{
    ...
    public static boolean Storm_Tornado_grabBlocks;
    ...
    public static float Wind_Particle_leaf_rate;
    ...
    public static boolean demoItems = false;
    ...
    public void preInit(FMLPreInitializationEvent event)
    {
        ...
        try
        {
            ...
            Storm_Tornado_grabBlocks = preInitConfig.get(Configuration.CATEGORY_GENERAL, "Storm_Tornado_grabBlocks", true).getBoolean(true);
            ...
            Wind_Particle_leaf_rate = (preInitConfig.get(Configuration.CATEGORY_GENERAL, "Wind_Particle_leaf_rate", 100).getInt() / 100F);
            ...
            Wind_Particle_waterfall = preInitConfig.get(Configuration.CATEGORY_GENERAL, "Wind_Particle_waterfall", true).getBoolean(true);
            Wind_Particle_snow = preInitConfig.get(Configuration.CATEGORY_GENERAL, "Wind_Particle_snow", false).getBoolean(false);
            ...
            if (c_CoroWeatherUtil.hasTropicraft()) {
            	...
            	waveRenderRange = preInitConfig.get(Configuration.CATEGORY_GENERAL, "waveRenderRange", 0).getInt();
            	...
            	demoItems = preInitConfig.get(Configuration.CATEGORY_GENERAL, "demoItems", false).getBoolean(false);
            	weatherItems = preInitConfig.get(Configuration.CATEGORY_GENERAL, "weatherItems", false).getBoolean(false);
            } else {
            	...
            	demoItems = preInitConfig.get(Configuration.CATEGORY_GENERAL, "demoItems", true).getBoolean(true);
            	weatherItems = preInitConfig.get(Configuration.CATEGORY_GENERAL, "weatherItems", true).getBoolean(true);
            }
    ...
    public void init()
    {
        ...
        for (int i = 0; i < maxRainDrops; i++) {
        	rainPositions[i] = Vec3.createVectorHelper((rand.nextFloat() * range) - (range/2), (rand.nextFloat() * range) - (range), (rand.nextFloat() * range) - (range/2));
        }
    ...
    public static void spin(Entity entity, WeatherEntityConfig conf, Entity entity1)
    {
        ...
        double grab = (10D / getWeight(entity1, true))/* / ((distY / maxHeight) * 1D)*/ * ((Math.abs((maxHeight - distY)) / maxHeight));
        float pullY = 0.0F;
    ...
    public static float getWeight(Entity entity1, boolean forTornado)
    {
        ...
        if (entity1 instanceof EntityLiving)
        {
            ...
            if (forTornado) {
            	...
            	return 0.1F;//1.0F + ((c_CoroWeatherUtil.getEntityAge((EntityLiving)entity1) + 150) / 50);
            } else {
            	return 500.0F + (entity1.onGround ? 2.0F : 0.0F) + ((c_CoroWeatherUtil.getEntityAge((EntityLiving)entity1) + 150) / 50);
            }
                ...
                for (int yy = curY - (hsize / 2); yy < curY + hsize; yy++)
                {
                    for (int zz = curZ - hsize; zz < curZ + hsize; zz++)
                    {
                        ...
                        if (block != null) {
                        	...
                        	//Waterfall
                        	if (Wind_Particle_waterfall && ((block.blockMaterial == Material.water))) {
    ...
    public static void tryParticles()
    {
        ...
        if (mc.objectMouseOver != null) {
        	...
        	if (id > 0 && Block.blocksList[id].blockMaterial == Material.wood) {
        		...
        		var5 = (Float)c_CoroWeatherUtil.getPrivateValueBoth(PlayerControllerMP.class, (PlayerControllerMP)ModLoader.getMinecraftInstance().playerController, "g", "curBlockDamageMP");
        ...
        if ((!Wind_Particle_leafs && !Wind_Particle_air && !Wind_Particle_sand && !Wind_Particle_waterfall)/* || weatherMan.wind.strength < 0.10*/)
        {
            return;
        }
        ...
        float particleCreationRate = Wind_Particle_leaf_rate;
                        ...
                        //{
                            ...
                            if (/*id == ((Block)p_blocks_leaf.get(i)).blockID*/block != null && (block.blockMaterial == Material.leaves || block.blockMaterial == Material.vine))
                            {
                            	...
                            	if (/*true || */worldRef.rand.nextInt(uh) == 0)
                                {
	                                ...
	                                if (Wind_Particle_leafs && (getBlockId(xx, yy - 1, zz) == 0 || getBlockId(xx - 1, yy, zz) == 0))
	                                {
                            ...
                            else if (Wind_Particle_waterfall && player.getDistance(xx,  yy, zz) < 16 && (block != null && block.blockMaterial == Material.water)) {
                            ...
                            else if (id == 0)
                            {
                            	...
                            	//Snow!
                            	if (false && Wind_Particle_snow && player.getDistance(xx, yy, zz) < 20 && (worldRef.rand.nextInt(100) == 0) && yy == ((int)player.posY+8) && temp <= 0.15F) {
                                ...
                                if (Wind_Particle_air && weatherMan.wind.strength > 0.05 && worldRef.canBlockSeeTheSky(curX, curY, curZ))
                                {
